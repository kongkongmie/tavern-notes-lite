...
const TEXT_ZH_CN = {
    ...
    backendNetworkHelp: '酒馆笔记后端没有响应。请确认 SillyTavern 已重启，server plugin 已安装到 plugins/tavern-notes-lite，并且 config.yaml 已开启 enableServerPlugins。',
...
const TEXTS = {
    ...
    'zh-TW': {
        ...
        backendNetworkHelp: '酒館筆記後端沒有回應。請確認 SillyTavern 已重啟、server plugin 已安裝到 plugins/tavern-notes-lite，且 config.yaml 已開啟 enableServerPlugins。',
...
只有點擊「預覽並儲存」或「另存為」才會產生主題檔案。`,
    ...
    en: {
        ...
        backendInstallTitle: 'One more step for Tavern Notes Lite',
        ...
        backendNetworkHelp: 'Tavern Notes Lite backend did not respond. Restart SillyTavern, make sure the server plugin is installed in plugins/tavern-notes-lite, and enableServerPlugins is enabled in config.yaml.',
        backendMissingHelp: 'Tavern Notes Lite backend was not found. Usually the backend installer has not been run, or SillyTavern was not restarted afterward.',
        backendRequestHelp: 'Tavern Notes Lite request failed: {message}',
...
Click Preview & save or Save as to create a theme file.`,
    ...
    ko: {
        ...
        backendInstallTitle: 'Tavern Notes Lite 설치가 한 단계 남았습니다',
        ...
        backendNetworkHelp: 'Tavern Notes Lite 백엔드가 응답하지 않습니다. SillyTavern을 다시 시작했고, server plugin이 plugins/tavern-notes-lite에 설치되어 있으며, config.yaml의 enableServerPlugins가 켜져 있는지 확인하세요.',
        backendMissingHelp: 'Tavern Notes Lite 백엔드를 찾을 수 없습니다. 보통 백엔드 설치기를 아직 실행하지 않았거나, 실행 후 SillyTavern을 다시 시작하지 않은 경우입니다.',
        backendRequestHelp: 'Tavern Notes Lite 요청 실패: {message}',
...
function renderCharacterOverview() {
    ...
    return `
        <section class="tnl-character-overview">
            ...
            <div class="tnl-character-featured">${renderCard(current, true)}</div>
            <div class="tnl-section-title">
        ...
        <section id="tavern-notes-lite-panel" aria-label="${htmlEscape(t('appName'))}">
            <header class="tnl-header">
                ...
                <div class="tnl-header-actions">
                    <button id="tavern-notes-lite-apple-mode-main" class="tnl-soft-button tnl-hidden" title="${htmlEscape(t('appleThemeModeTitle'))}" aria-label="${htmlEscape(t('appleThemeModeTitle'))}">
                        <i class="fa-solid fa-moon"></i><span>${htmlEscape(t('appleThemeNight'))}</span>
                    </button>
                    ...
                    <button id="tavern-notes-lite-floor-capture-open" class="tnl-soft-button ${state.showFloorCaptureButton ? 'active' : ''}" title="${htmlEscape(t('floorCaptureEntryTitle'))}" aria-label="${htmlEscape(t('floorCaptureEntryTitle'))}">
                        <i class="fa-solid fa-file-lines"></i><span>${htmlEscape(t('floorCaptureEntry'))}</span>
                    </button>
                    <button id="tavern-notes-lite-theme" class="tnl-soft-button" title="${htmlEscape(t('openThemePanel'))}" aria-label="${htmlEscape(t('openThemePanel'))}">
                        <i class="fa-solid fa-palette"></i><span>${htmlEscape(t('theme'))}</span>
                    </button>
                    <button id="tavern-notes-lite-export" class="tnl-icon-button" title="${htmlEscape(t('exportNotes'))}" aria-label="${htmlEscape(t('exportNotes'))}">
            ...
            <div id="tavern-notes-lite-export-menu" aria-hidden="true">
                <div class="tnl-export-card">
                    ...
                    <button class="tnl-export-choice" data-format="both" title="JSON + TXT"><i class="fa-solid fa-layer-group"></i><span>${htmlEscape(t('exportBoth'))}</span></button>
...
function bindEvents() {
    ...
    document.querySelectorAll('#tavern-notes-lite-export-menu .tnl-export-choice').forEach(button => {
...
function paintTheme(theme) {
    const clean = applyAppleGlassMode(normalizeTheme(theme));
    const panel = document.querySelector('#tavern-notes-lite-panel');
    if (panel) {
...
function updateAppleThemeModeButton() {
    ...
    const isNight = state.appleGlassMode === 'night';
    for (const button of buttons) {
        button.classList.toggle('tnl-hidden', !isApple);
        button.classList.toggle('active', isApple);
        button.title = t('appleThemeModeTitle');
        button.setAttribute('aria-label', t('appleThemeModeTitle'));
        button.querySelector('i')?.classList.toggle('fa-sun', isNight);
        button.querySelector('i')?.classList.toggle('fa-moon', !isNight);
        button.querySelector('span')?.replaceChildren(document.createTextNode(isNight ? t('appleThemeDay') : t('appleThemeNight')));
    }
}
...
function updateFloatingLauncher() {
    ...
    if (!launcher) {
        ...
        document.querySelector('#tavern-notes-lite-floating-capture')?.addEventListener('click', () => captureSelection().catch(error => notify(error.message, 'error')));
    }
