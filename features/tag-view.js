...
export function createTagView({
    ...
    function allTags(state) {
        ...
        const append = tag => {
            ...
            if (byKey.has(key)) {
                ...
                current.count = Math.max(Number(current.count || 0), Number(tag.count || 0));
                return;
            }
            const value = { ...tag, name, count: Number(tag.count || 0) };
            byKey.set(key, value);
            tags.push(value);
        };
        state.tags.forEach(append);
        state.recentTags.forEach(name => append({ name, count: 0 }));
        return tags;
    }
    ...
    function homeTags(state) {
        ...
        [...tags].sort((a, b) => Number(b.count || 0) - Number(a.count || 0) || a.name.localeCompare(b.name)).forEach(append);
    ...
    function renderShelf() {
        ...
        shelf.innerHTML = `<button class="${ui('tag-filter')} ${selectors.prefix}-tag-library-open" type="button"><i class="fa-solid fa-tags"></i><span>${escapeHtml(translate('allTags'))}</span><small>${tags.length}</small></button>${state.activeTag ? `<button class="${ui('tag-filter')} ${selectors.prefix}-tag-clear active" type="button" data-tag=""><i class="fa-solid fa-xmark"></i><span>${escapeHtml(translate('clearTagFilter'))}</span></button>` : ''}${homeTags(state).map(tag => `<button class="${ui('tag-filter')} ${state.activeTag === tag.name ? 'active' : ''}" type="button" data-tag="${escapeHtml(tag.name)}"><span>${escapeHtml(tag.name)}</span><small>${tag.count}</small></button>`).join('')}${!tags.length ? `<div class="${selectors.prefix}-tag-shelf-empty"><i class="fa-solid fa-pen-to-square"></i><span>${escapeHtml(translate('tagShelfEmpty'))}</span></div>` : ''}`;
    }
